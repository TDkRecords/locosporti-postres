...
<div class="space-y-6 p-4 sm:p-6">
    <header class="rounded-3xl bg-white p-5 shadow-sm">
        <p class="text-sm font-semibold uppercase tracking-[0.2em] text-[#CDB9FE]">Perfil</p>
    ...
    {#if successMsg}
        <div class="rounded-2xl bg-green-100 px-4 py-3 text-sm text-green-700 font-semibold">
            {successMsg}
        </div>
    {/if}
    {#if errorMsg}
        ...
        <section class="rounded-3xl bg-white p-6 shadow-sm">
            <div class="flex flex-col items-center gap-4 text-center">
                ...
                <h2 class="text-2xl font-bold text-gray-900">
                    {$clientProfile?.nombre || ""} {$clientProfile?.apellido || $currentUser?.displayName || "Cliente"}
                </h2>
                ...
                <div class="rounded-3xl bg-[#FFFB96]/60 p-4">
                    ...
                    <dl class="mt-3 space-y-2 text-sm text-gray-700">
                        <div class="flex gap-2">
                            ...
                            <dd>{$clientProfile?.nombre || "—"} {$clientProfile?.apellido || ""}</dd>
                        </div>
                        <div class="flex gap-2">
                        ...
                        <div class="flex gap-2">
                            ...
                            <dd class="break-all">{$currentUser?.email || "—"}</dd>
                        </div>
                        <div class="flex gap-2">
                            <dt class="font-semibold w-24 shrink-0">Dirección:</dt>
                            <dd>{$clientProfile?.direccion || "Sin dirección"}</dd>
                        </div>
                        {#if $clientProfile?.sugerencias}
                            <div class="flex gap-2">
                                <dt class="font-semibold w-24 shrink-0">Notas:</dt>
                                <dd class="italic text-gray-500">{$clientProfile.sugerencias}</dd>
                            </div>
                        {/if}
                    </dl>
                </div>
                ...
                {#if $clientProfile?.changeLog && $clientProfile.changeLog.length > 0}
                    <div class="rounded-3xl bg-[#FFE28A]/60 p-4">
                        ...
                        <div class="mt-3 space-y-2">
                            {#each $clientProfile.changeLog.slice().reverse() as log}
                                <p class="text-xs text-gray-500">
                                    {new Date(log.at).toLocaleString("es-CO")} ·
                                    Cambió: {Object.keys(log.cambios || {}).join(", ")}
                                </p>
                            {/each}
                        </div>
                    </div>
        ...
        <section class="space-y-6">
            <div class="rounded-3xl bg-white p-6 shadow-sm">
                ...
                <div class="mt-6 space-y-3">
                    ...
                    <button
                        type="button"
                        onclick={() => { showDeactivateConfirm = true; errorMsg = ""; }}
                    ...
                    <button
                        type="button"
                        onclick={() => { showDeleteConfirm = true; errorMsg = ""; }}
...
{#if showEditForm}
    <div class="fixed inset-0 z-50 flex items-end justify-center bg-black/40 sm:items-center">
        <div class="w-full max-w-lg rounded-t-3xl bg-white p-6 shadow-2xl sm:rounded-3xl sm:m-4 max-h-[90vh] overflow-y-auto">
            <h2 class="text-xl font-bold text-gray-900">Actualizar información</h2>
            <form onsubmit={handleActualizarPerfil} class="mt-4 space-y-3">
                <div class="grid gap-3 sm:grid-cols-2">
                    ...
                    <div>
                        <label class="mb-1 block text-sm font-semibold text-gray-700">Apellido</label>
                        <input
                ...
                <div>
                    <label class="mb-1 block text-sm font-semibold text-gray-700">Edad</label>
                    <input
                ...
                <div>
                    <label class="mb-1 block text-sm font-semibold text-gray-700">Calle / Carrera</label>
                    <input
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    <div>
                        <label class="mb-1 block text-sm font-semibold text-gray-700"># de calle</label>
                        <input
                    ...
                    <div>
                        <label class="mb-1 block text-sm font-semibold text-gray-700">Complemento</label>
                        <input
                ...
                <div>
                    <label class="mb-1 block text-sm font-semibold text-gray-700">Casa / Apto / Edificio</label>
                    <input
                ...
                <div>
                    <label class="mb-1 block text-sm font-semibold text-gray-700">Sugerencias para llegar</label>
                    <textarea
...
{#if showDeactivateConfirm}
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
        <div class="w-full max-w-sm rounded-3xl bg-white p-6 shadow-2xl">
            ...
            <p class="mt-2 text-sm text-gray-500">
                Tu cuenta quedará inactiva. Podrás reactivarla iniciando sesión nuevamente.
            </p>
...
{#if showDeleteConfirm}
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
        <div class="w-full max-w-sm rounded-3xl bg-white p-6 shadow-2xl">
            ...
            <p class="mt-2 text-sm text-gray-500">
                Tu cuenta será marcada como eliminada. El administrador aún tendrá acceso
                a tu historial para gestión interna.
            </p>
            <div class="mt-6 flex gap-3">
                <button
                    type="button"
